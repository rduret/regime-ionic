...
export class AddIngredientPage implements OnInit {
  ...
  async takePhoto() {
    ...
    reader.onloadend = (evt) => {
      let blob = new Blob([evt.target.result], { type: capturedPhoto.format });
      this.photo = new File([blob], 'test', {type: capturedPhoto.format})
      console.log(this.photo);
      console.log(this.ingredientsService.uploadFile(this.photo));
    }
    reader.onerror = function (e) {
        console.log("Failed file read: " + e.toString());
    };
    reader.readAsArrayBuffer(this.b64toBlob(capturedPhoto.dataUrl));


    this.photoShow = capturedPhoto.dataUrl;
