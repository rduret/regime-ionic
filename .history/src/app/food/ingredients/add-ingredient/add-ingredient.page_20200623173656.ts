...
export class AddIngredientPage implements OnInit {
  ...
  async openGallery() {
    ...
    let file = this.urltoFile(capturedPhoto.dataUrl, 'nomIngrédient', capturedPhoto.format);
    this.photoShow = capturedPhoto.webPath;

    this.ingredientsService.uploadFile(file);
  }
  ...
  urltoFile(url, filename, mimeType){
    ...
    (fetch(url)
      ...
      .then((buf) => {file = new File([buf], filename,{type:mimeType});})
    );
    return file;
}
/*   openGallery () {
