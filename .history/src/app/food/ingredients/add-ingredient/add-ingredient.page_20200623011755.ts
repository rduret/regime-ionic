...
export class AddIngredientPage implements OnInit {
  ...
  //UPLOAD FILE
  photoFileUri: string;                                       //Uri de la photo stockée sur le tel
  ...
  onUploadFile(){
    ...
    this.ingredientsService.uploadFile(this.photoShow).then(
      (url: string) => {
...
/*     this.file.resolveLocalFilesystemUrl(this.photoFileUri)
  ...
  onAddIngredient() {
    this.onUploadFile();      //Upload ne fonctionne pas / essayer base64
