...
export class EditIngredientPage implements OnInit {
  ...
  checkIngredientValidator(listIngredients: Ingredient[], editedIngredient: Ingredient): ValidatorFn{            
    return (ingredientName: AbstractControl): {[key: string]: boolean} | null => {
      ...
      {
        let newIngredientName = ingredientName.value.toLocaleLowerCase();
        let currentIngredientName = editedIngredient.name.toLocaleLowerCase()
        let identical:Boolean = false;
        
        if(listIngredients)
        {
          listIngredients.forEach(function(ingredient){
            if(newIngredientName == ingredient.name.toLocaleLowerCase() && newIngredientName !== currentIngredientName)        //Si l'ingrédient du tab a le même nom que celui tapé dans l'input 
