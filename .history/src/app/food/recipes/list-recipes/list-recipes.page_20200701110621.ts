...
export class ListRecipesPage implements OnInit {
  ...
  getSizeIngredientsToDelete(){ return this.recipesToDelete.length; }
      ...
      buttons: [
        {
          ...
          text: 'Supprimer',
          handler: () => {
            let numberIngr = this.recipesToDelete.length;
            this.deleteRecipes();
  ...
  async presentDeleteToast(numberRecipes: number) {
    ...
    let message: string;
    if(numberRecipes > 1)
      message = numberRecipes + ' ingrédients supprimés !';
    else
      message = numberRecipes + ' ingrédient supprimé !';
